...
internal sealed class ServiceTypeService : IServiceTypeService
{
    ...
    public  async Task<APIResponse<IEnumerable<ServiceTypeResponse>>> GetAllByBrandId(Guid brandId)
    {
       var services =   await repository.FindByAsync(x => x.BrandId == brandId);
        if(!services.Any())
            return APIResponse<IEnumerable<ServiceTypeResponse>>.ErrorResponse("No Service Found", APIStatusCodes.NotFound);
