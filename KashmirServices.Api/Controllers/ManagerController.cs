...
public class ManagerController : ApiController
{
    ...
    [HttpGet("categories/managerid")]
    public async Task<IResult> GetMyCategories([FromQuery] Guid? managerid)
    {
        var result = await service.GetCategoriesByManager(managerid);
    ...
    [HttpGet("all-engineers/{categoryid:guid}")]
    public async Task<IResult> GetAllEngineersByCategoryId(Guid categoryid)
    {
        var result = await jobRoleService.GetAllEngineersByCategoryId(categoryid);

        return this.APIResult(result);
    }


    [HttpGet("all-engineers/managerid")]
    public async Task<IResult> GetAllEngineersByManagerId([FromQuery]Guid? managerid)
    {
        var result = await jobRoleService.GetAllEngineersByManagerIdAsync(managerid);
