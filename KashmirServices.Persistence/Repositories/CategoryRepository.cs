...
public class CategoryRepository : BaseRepository<Category>, ICategoryRepository
{
    ...
    public async Task<IEnumerable< EngineerCategoryResposne>> GetCategoriesByEngineerId(Guid id)
    {


        string engineerCategories = $@" SELECT C.Id ,C.[Name],[Description], F.FilePath
							                    FROM Categories C
                                                ...
                                                LEFT JOIN AppFiles F
                                                ON U.ID = F.EntityId 
                                                WHERE C.IsDeleted = 0 AND J. EngineerId = @id
							                    ORDER BY C.CreatedOn DESC ";
        return await QueryAsync<EngineerCategoryResposne>(engineerCategories, new { id });
        }
    }
